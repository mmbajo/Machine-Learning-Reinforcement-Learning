...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    """
    # TODO Your code here
    #q_value = np.zeros((NUM_ACTIONS, NUM_OBJECTS))
    if np.random.uniform() >= epsilon:
        # for action in range(NUM_ACTIONS):
        #     for objects in range(NUM_OBJECTS):
        #print(theta.shape, state_vector.shape)
        q_value = theta @ state_vector
        action_index, object_index = index2tuple(np.argmax(q_value))
    else:
        action_index, object_index = np.random.choice(np.arange(NUM_ACTIONS)), np.random.choice(np.arange(NUM_OBJECTS))
    ...
    #     for action in range(NUM_ACTIONS):
    #         for objects in range(NUM_OBJECTS):
    #             q_value[action, objects] = (theta @ state_vector)[tuple2index(action, objects)]
    #     action_index, object_index = np.unravel_index(np.argmax(q_value), q_value.shape)
    # else:
    #     action_index, object_index = np.random.choice(np.arange(NUM_ACTIONS)), np.random.choice(np.arange(NUM_OBJECTS))
    # return (action_index, object_index)
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    #     theta += ALPHA * (reward + GAMMA * (np.max(q_value_next)-q_value) * current_state_vector)  # TODO Your update here
    # else:
    #     theta += ALPHA * (reward + GAMMA * 0)
    q_value = (theta @ current_state_vector)[tuple2index(action_index, object_index)]
